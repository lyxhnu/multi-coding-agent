...
├─ request_start   { model, context, reasoning } // Final transformed LLM request
...
├─ request_start      { model, context, reasoning }
...
| `request_start` | Final model, reasoning level, and transformed `Context` immediately before request authentication and provider dispatch |
...
`runState` is a discriminated union. Active runs expose their `runId`, turn number, and one explicit phase:
`preparing`, `requesting`, `streaming`, `processing_response`, `executing_tools`, `between_turns`, `cancelling`,
or `settling`. Streaming messages and pending tool-call IDs exist only on the phases where they are valid.

The state returns to `idle` only after the run fully settles, including awaited `agent_end` subscribers.
