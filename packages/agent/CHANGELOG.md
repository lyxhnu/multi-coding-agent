...
- Added append-only context mode (`AgentOptions.appendOnlyContext`, `AppendOnlyContextManager`), which reuses a stable system-prompt/tool prefix and only appends new messages so provider prompt caches keep hitting. An in-place message rewrite now re-sends only from the changed message instead of the whole transcript.
- Added the `request_start` agent event, exposing the final transformed LLM context and request model before authentication and provider dispatch.
