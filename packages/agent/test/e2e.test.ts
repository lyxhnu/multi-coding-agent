	...
	agent.subscribe((event) => {
		if (event.type === "tool_execution_start" || event.type === "tool_execution_end") {
			pendingToolCallsDuringEvents.push({
				type: event.type,
				ids:
					agent.state.runState.status === "running" && agent.state.runState.phase.type === "executing_tools"
						? [...agent.state.runState.phase.pendingToolCallIds]
						: [],
			});
		}
	});
...
async function abortExecution(model: Model<string>) {
	...
	expect(agent.state.runState).toEqual({
		status: "idle",
		lastOutcome: { type: "aborted", message: lastMessage.errorMessage },
	});
}
