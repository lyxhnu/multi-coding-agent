...
describe("Agent", () => {
	...
	it("should create an agent instance with default state", () => {
		...
		expect(agent.state.runState).toEqual({ status: "idle" });
	});
	...
	it("emits full lifecycle events for thrown run failures", async () => {
		...
		expect(agent.state.runState).toEqual({
			status: "idle",
			lastOutcome: { type: "failed", message: "provider exploded" },
		});
	});
	...
	it("should await async subscribers before prompt resolves", async () => {
		...
		expect(agent.state.runState.status).toBe("idle");
	});
	...
	it("should reject reset while a run is active", async () => {
		const agent = new Agent({
			streamFn: (_model, _context, options) => {
				const stream = new MockAssistantStream();
				queueMicrotask(() => {
		...
		const prompt = agent.prompt("running");
		await new Promise((resolve) => setTimeout(resolve, 10));
		expect(() => agent.reset()).toThrow("Cannot reset while the agent is processing.");
		agent.abort();
		await prompt;
	});
