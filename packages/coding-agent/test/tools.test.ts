...
describe("Coding Agent Tools", () => {
	...
	describe("write tool", () => {
		...
		/**
		 * Steers the model toward edit for existing files. Benchmark runs showed single write calls taking
		 * 2.7-3.6 minutes each (the model emitting a whole file token by token) and one task rewriting the
		 * same file twice before timing out. Asserted so the guidance is not quietly dropped later.
		 */
		it("tells the model to prefer edit over rewriting an existing file", () => {
			const guidelines = (createWriteToolDefinition(process.cwd()).promptGuidelines ?? []).join(" ");
			expect(guidelines).toContain("edit");
			// The reason has to travel with the instruction; a bare prohibition moves the model less.
			expect(guidelines).toMatch(/slower|token per line/);
		});
	});
