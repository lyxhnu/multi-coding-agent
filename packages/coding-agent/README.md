...
- Custom sub-agent and planning workflows
- Custom compaction and summarization
- Custom permission policies and path protection
...
**Minimal by default.** The model starts with filesystem/process tools and a scoped history reader. Optional built-ins do not silently enlarge that capability set; activate them with `--tools` or SDK configuration.

**Approval is not isolation.** Risky built-in process, network, and external-system tool calls require approval in interactive/RPC hosts and fail closed without an approval channel. Extensions execute as trusted local code. Use a container or VM when an operating-system security boundary is required.

**Composable workflows.** Built-in task, plan, todo, memory, LSP, web, and MCP primitives can be selected explicitly or replaced with extensions and packages.
...
| `TAVILY_API_KEY` | Register the built-in Tavily-backed `web_search` tool; omitted when unset and not activated implicitly |
