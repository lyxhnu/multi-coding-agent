...
import { type AgentMessage, applyRedactions, type StreamFn, type ThinkingLevel } from "@earendil-works/pi-agent-core";
import {
...
export async function completeSummarization(
	...
	if (
		...
		throw new Error("Summary request exceeds context budget");
	}
	...
	if (response.stopReason !== "stop" || !contentText(response.content).trim()) {
		throw new Error(
			`Summarization failed: ${response.stopReason === "stop" ? "empty summary" : (response.errorMessage ?? response.stopReason)}`,
		);
	}
	return response;
}
