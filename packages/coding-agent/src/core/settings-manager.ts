...
import { type ResolvedSandboxSettings, resolveSandboxSettings, type SandboxSettings } from "./sandbox/types.ts";
import { type ReminderPolicy, resolveReminderPolicy } from "./todo/reminder-policy.ts";
...
export interface ContextSettings {
	/**
	 * Reuse a stable byte prefix and only append new messages each turn, so the provider's prompt
	 * cache keeps hitting instead of being invalidated by a full re-serialization. Biggest win on
	 * long sessions and local backends, where a full re-prefill is expensive.
	 *
	 * Off by default: it changes the exact bytes sent to providers, so it opts in per user.
	 */
	appendOnly?: boolean; // default: false
}

export interface ReminderTodoNudgeSettings {
	enabled?: boolean; // default: true
	turnsSinceTodoWrite?: number; // default: 3
	turnsBetweenReminders?: number; // default: 5
}

export interface ReminderTodoGateSettings {
	enabled?: boolean; // default: false (opt-in)
	maxFiresPerPrompt?: number; // default: 2
}

export interface ReminderSettings {
	enabled?: boolean; // default: true
	todoNudge?: ReminderTodoNudgeSettings;
	todoGate?: ReminderTodoGateSettings;
}
...
export interface BashBackgroundSettings {
	enabledBackground?: boolean; // default: true
	autoBackgroundOnTimeout?: boolean; // default: false
	foregroundBlockBudgetMs?: number; // default: 15000
	maxBlockUntilMs?: number;
	allowBackgroundOperator?: boolean; // default: true
	surfaceBgCompletionReminders?: boolean; // default: true
	/** Migration flag (spec 15.1): interpret bash's `timeout` input as seconds (pi's original unit) instead of milliseconds (Grok-aligned default). Default: false. */
	legacyTimeoutSeconds?: boolean;
}

export interface PlanModeSettings {
	/** Timeout for the exit_plan_mode approval PendingInteraction, in ms. Default: 120000 (Grok-aligned). */
...
export class SettingsManager {
	...
	getAppendOnlyContext(): boolean {
		return this.settings.context?.appendOnly ?? false;
	}

	getPermissionMode(): PermissionMode {
		return this.settings.permissions?.mode ?? "default";
	}

	isPermissionAuditEnabled(): boolean {
		return this.settings.permissions?.audit ?? true;
	}

	getPermissionAllowRules(): PermissionRule[] | undefined {
		return this.settings.permissions?.allow;
	}

	getPermissionDenyRules(): PermissionRule[] | undefined {
		return this.settings.permissions?.deny;
	}

	/** Grok-aligned TodoNudge/TodoGate reminder policy. */
	...
	/** Grok-aligned memory system settings (spec 10, section 14 defaults: enabled=false, scope=project, globalAutoWrite=false). */
	getMemorySettings(): {
		enabled: boolean;
	...
	/** Grok-aligned sandbox profile settings (spec 12, section 14 default: profile=workspace, mode=best-effort). */
	getSandboxSettings(): ResolvedSandboxSettings {
		return resolveSandboxSettings(this.settings.sandbox);
	}

	/**
	 * Whether the user/project explicitly set a `sandbox` block (as opposed to relying on the resolved
	...
	getBashBackgroundSettings(): Required<BashBackgroundSettings> {
		return {
			enabledBackground: this.settings.bash?.enabledBackground ?? true,
			autoBackgroundOnTimeout: this.settings.bash?.autoBackgroundOnTimeout ?? false,
			foregroundBlockBudgetMs: this.settings.bash?.foregroundBlockBudgetMs ?? 15_000,
			maxBlockUntilMs: this.settings.bash?.maxBlockUntilMs ?? 0,
			allowBackgroundOperator: this.settings.bash?.allowBackgroundOperator ?? true,
			surfaceBgCompletionReminders: this.settings.bash?.surfaceBgCompletionReminders ?? true,
			legacyTimeoutSeconds: this.settings.bash?.legacyTimeoutSeconds ?? false,
		};
	}
