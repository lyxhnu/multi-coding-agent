...
Every model-requested tool call is checked after extension argument mutation and schema validation. The policy classifies built-in calls by effect:

- read-only and session-local effects are allowed
- workspace edits follow the selected permission mode
- risky shell commands and built-in process, network, or external-system effects require approval
- explicit deny rules and workspace symlink escapes are denied before mode policy

Interactive and RPC hosts use their confirmation channel for `ask` decisions. SDK or disconnected hosts fail closed. `dontAsk` denies effects that are not pre-approved; `bypassPermissions` allows them except for explicit deny rules and hard denies. Allow and deny rules match `${toolName}:${operation}` and deny rules take precedence.

The permission guard controls model-requested built-in tool calls. Extensions are trusted when loaded and are not treated as a separate per-call security boundary. The guard also does not constrain shell commands the user invokes directly or the operating-system privileges of an approved tool.

## No Built-in Sandbox

Pi does not include a built-in operating-system sandbox. Once a tool call is permitted, built-in tools can read files, write files, edit files, and run shell commands with the permissions of the pi process. Extensions are TypeScript modules that run with the same permissions. Package installs, shell commands, language servers, test commands, and other developer tools behave as ordinary local processes.
