...
| `TAVILY_API_KEY` | Enable the built-in Tavily-backed `web_search` tool; omitted when unset |
...
Set `TAVILY_API_KEY` before starting `pi` to register the built-in `web_search` tool. If the variable is not set, the tool is not exposed to the model.
