...
import { calculateContextBudget } from "../utils/estimate.ts";
...
export function clampMaxTokensToContext(model: Model<Api>, context: Context, maxTokens: number): number {
	...
	const available =
		calculateContextBudget(model, context, { outputReserveTokens: maxTokens }).availableOutputTokens ?? maxTokens;
