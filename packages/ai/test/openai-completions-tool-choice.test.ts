...
describe("openai-completions tool_choice", () => {
	...
	it("omits tool_stream for unsupported z.ai models", async () => {
		...
		// Every cataloged z.ai model supports tool_stream now; opt out explicitly (falsy means unsupported).
	...
	it("respects explicit z.ai tool_stream compat override", async () => {
		const stripped = getModel("zai", "glm-4.7")!;
		const baseModel = { ...stripped, compat: { ...stripped.compat, zaiToolStream: false } };
		const model = {
			...baseModel,
			compat: {
				...baseModel.compat,
	...
	it("sends max_tokens for OpenCode completions models", async () => {
		const cases = [getModel("opencode-go", "kimi-k2.6")!, getModel("opencode", "kimi-k2.5")!] as const;
