...
mkdir -p "$test_root/home/.config" "$test_root/tmp" "$test_root/cache/npm" "$test_root/bin"
...
# Preserve the host-provisioned search tools inside the isolated HOME. Some
# coding-agent tests invoke fd/rg through pi's tool bootstrap; downloading them
# during a no-network test run would turn an environment issue into a regression.
for tool in fd rg; do
	host_tool="$HOME/.pi/agent/bin/$tool"
	if [[ -x "$host_tool" ]]; then
		cp "$host_tool" "$test_root/bin/$tool"
