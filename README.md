...
- 每轮工具调用后执行上下文守卫，支持有界 shake、压缩和救援流程，防止循环无限压缩或重复执行工具。

### 2. Shake、压缩与历史回读

- 增加零模型调用的 Shake：将超大的工具结果或文本块替换为带来源的占位符，并把变更持久化到会话日志。
- 修复 split-turn、重复压缩和空摘要场景中的摘要继承问题；无效、截断、取消或空摘要不会提交。
- 增加两阶段增量压缩 checkpoint：后台预总结旧前缀，最终压缩只处理未覆盖尾部，并校验分支、来源指纹和保留边界。
- 增加只读 `history_get` 工具，按 entry/block/offset/limit 分页读取被 Shake 隐藏的已保存文本，不重新执行原工具。
...
- 当前记忆与上下文专项验证为：`npm run check` 通过；496 项定向测试通过，12 项按真实模型授权或 OS sandbox 条件跳过。
- 全仓离线回归仍包含缺失构建产物、Windows 平台差异及尚未完全定位的失败，不能把专项通过表述为全仓完成验收。
